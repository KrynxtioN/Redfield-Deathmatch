	...
	if(#result >= 1)then
		...
		if(BannTime ~= 0)then
			if(getRealTime().timestamp >= BannTime)then
				...
				kickPlayer(source,"System","Du bist noch bis zum "..TimestampToDate(getPlayerData("bannedplayers","Username",getPlayerName(source),"BannTime")).." gebannt! Grund: "..getPlayerData("bannedplayers","Username",getPlayerName(source),"BannTime"))
			end
		else
			kickPlayer(source,"System","Du wurdest permanent gebannt! Grund: "..getPlayerData("bannedplayers","Username",getPlayerName(source),"Grund"))
		end
	end
end)
